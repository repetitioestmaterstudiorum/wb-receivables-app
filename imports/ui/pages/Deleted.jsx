...
const Deleted = () => {
	...
	const handleRestore = () => {
		...
		if (checkedInvoices.length !== 0) {
			checkedInvoices.forEach(element => Meteor.call('markInvoiceNotDeleted', element))
			setCheckedInvoices([])
		}
		if (checkedPayments.length !== 0) {
			checkedPayments.forEach(element => Meteor.call('markPaymentNotDeleted', element))
			setCheckedPayments([])
		}
	}
	...
	return (
		<Container>
			<button className='btn btn-outline-success btn-sm mb-2 mr-2' onClick={handleRestore}>
				Restore
			</button>
			<Row>
				<Col sm={7}>
