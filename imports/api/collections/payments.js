...
Meteor.methods({
	markPaymentDeleted(paymentId) {
		...
		PaymentsCollection.update({ _id: paymentId }, { $set: { isDeleted: true } })
	},
	markPaymentNotDeleted(paymentId) {
		...
		check(paymentId, String)
		PaymentsCollection.update({ _id: paymentId }, { $set: { isDeleted: false } })
	},
	...
	paymentIsNotPaired(paymentId) {
		...
		check(paymentId, String)
		PaymentsCollection.update({ _id: paymentId }, { $set: { isPaired: false } })
	},
})
